...
export default function BuyPage() {
  ...
  return (
    <div className="p-6 max-w-lg mx-auto min-h-screen relative">
      ...
      <StaggerContainer staggerDelay={0.06} delayChildren={0.08} className="space-y-3">
        {/* Pay with UPI - Recommended */}
        <StaggerItem>
          <motion.button
            onClick={() => setActiveView('upi')}
            className="w-full card-gold rounded-2xl p-4 flex items-center gap-4 text-left hover:border-gold-500 dark:hover:border-gold-400 transition-all"
            whileTap={{ scale: 0.98 }}
        ...
        <StaggerItem>
          <motion.button
            ...
            className="w-full card rounded-2xl p-4 flex items-center gap-4 text-left hover:border-gold-500/50 dark:hover:border-gold-400/50 transition-all"
            whileTap={{ scale: 0.98 }}
            transition={SPRING.snappy}
          >
